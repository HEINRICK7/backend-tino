...
public final class GetGoodsReceiptPreview {
    ...
    public PreviewSnapshot execute(UUID userId, BusinessId businessId, UUID previewId) { return authorization.execute(userId, businessId, authorized -> previews.findPreview(authorized, previewId, false).orElseThrow(() -> new ReceivingException(ReceivingErrorCode.NFE_NOT_FOUND, "preview not found", false, 404))); }
    public PreviewSnapshot executeByDocument(UUID userId, BusinessId businessId, UUID documentId) { return authorization.execute(userId, businessId, authorized -> previews.findPreviewByDocument(authorized, documentId).orElseThrow(() -> new ReceivingException(ReceivingErrorCode.NFE_NOT_FOUND, "preview not found", false, 404))); }
}
