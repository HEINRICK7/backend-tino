...
public final class ConfirmGoodsReceipt {
    ...
    public UUID execute(UUID userId, BusinessId businessId, UUID previewId, long expectedVersion, List<Decision> decisions) {
        return authorization.execute(userId, businessId, authorized -> {
            var preview = receiving.findPreview(authorized, previewId, true).orElseThrow(() -> new ReceivingException("preview not found"));
            var existing = receiving.findReceiptByPreview(authorized, previewId); if (existing.isPresent()) return existing.get();
            if (preview.version() != expectedVersion || !preview.status().equals("READY") && !preview.status().equals("REVIEW_REQUIRED")) throw new ReceivingException("preview is obsolete or not confirmable");
            var document = fiscal.find(authorized, preview.documentId()).orElseThrow(() -> new ReceivingException("fiscal document not found"));
            if (document.fiscalStatus() == FiscalStatus.CANCELLED || document.fiscalStatus() == FiscalStatus.DENIED) throw new ReceivingException("cancelled or denied fiscal document cannot enter stock");
            ...
            for (var item : preview.items()) {
                ...
                if (decision.action() == Action.USE_EXISTING) productId = Objects.requireNonNull(decision.productId(), "productId");
                if (productId == null) throw new ReceivingException("each non-ignored item needs a product decision");
                ...
                if (factor == null) { if (baseUnit != null && baseUnit.equalsIgnoreCase(item.purchaseUnit())) factor = BigDecimal.ONE; else throw new ReceivingException("packaging conversion is required"); }
                if (factor.signum() <= 0) throw new ReceivingException("conversion factor must be positive");
            ...
            receiving.markPreviewConfirmed(authorized, previewId, now); return receiptId;
        });
    }
