...
public final class GetNfeDocument {
    ...
    public NfeDocumentSnapshot execute(UUID userId, BusinessId businessId, UUID documentId) { return authorization.execute(userId, businessId, authorized -> documents.find(authorized, documentId).orElseThrow(() -> new IllegalArgumentException("fiscal document not found"))); }
}
